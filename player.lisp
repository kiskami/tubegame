...
(defun show-player (player camnode level)
 ...
 ;     (format t "First startpos from level: ~A~%" startpos)
    (if (null startpos)
	(setf startpos *PLAYER-INITIAL-POS*))
    (format t "Player startpos: ~A~%" startpos)
    (llgs-engine-cl:render-setscenenodepos (playerdata-node player) (first startpos) (second startpos) (third startpos))
